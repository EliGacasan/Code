...
source("Data and Scripts/Analysis.R")
...
# For the purposes of known warnings that don't mean very much


## Define variables for UI ## 
# Codebook
# input.codebook.sections <- c("Definitions of ACLED Event and Sub-Event Types", "Introduction and Brief Description",
#                              "Violent Events-Battles")
# Fry plot
# input.fryPlotChoices <- c("event_type", "n")
# names(input.fryPlotChoices) = c("Event Type", "Number of events")
# Function choices
# input.function <- c("K-Function","Inhomogeneous K-Function","L-Function", "Inhomogeneous Pair Correlation Function",
#                     "Sector K-Function", "Pair Correlation Function")
# Edge Correction
# input.edgecorrection <- c("none", "border", "bord.modif", "isotropic", "Ripley", "translate", "translation", "rigid", 
#                           "periodic", "good", "best", "all")
# names(input.edgecorrection) <- c("None", "Border", "Modified Border", "Isotropic", "Ripley", "Translate", "Translation",
#                                  "Rigid", "Periodic", "Good", "Best", "All")

# Methodology 
# input.methodology.exploratory <- c("Point Pattern: Intensity", "Point Pattern: Correlation", 
#                                    "Point Pattern: Spacing")
# Variables for input choices
# data.event.types <- unique(data$event_type)
# data.admin1.zones <- unique(data$admin1)
# map.types <- c("Basemap", "3-D Hexagonal", "Heatmap", "3-D Chloropleth")
# max.date <- max(data$event_date)
# min.date <- min(data$event_date)

# Key for mapdeck
# key <- "pk.eyJ1IjoiZ21lMTQ3OSIsImEiOiJjbGU2YmFuNDcwZjduM25wYnJ2enl1Y3lwIn0.je7BWCeWm_QMb-qP_T8bfg"
